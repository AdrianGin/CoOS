...
void SystemInit (void) 
{
    ...
    *(uint32_t *)0x40006C18 = 0x00008000;
    ...
    uint32_t * pStartOfStack;
    uint32_t * pEndOfStack;
    ...
    pStartOfStack = (uint32_t *) __Vectors;
    pEndOfStack = pStartOfStack - STACK_SIZE/4;
#elif defined ( __GNUC__ ) || defined ( __SES_ARM )

    extern uint32_t * __StackTop;                  /**< The initialization vector holds the address to __initial_sp that will be used when fetching the stack. */
    extern uint32_t * __HeapLimit;
    pStartOfStack = (uint32_t *)&__StackTop;
    pEndOfStack = (uint32_t *)&__HeapLimit;
#endif

    
/*  Also update in arm_startup_nrf51.s with stack size */
#define STACK_SIZE 1748                           //2048 - haven't seen much more than a 1k being used
...
//    NRF_MPU->DISABLEINDEBUG = MPU_DISABLEINDEBUG_DISABLEINDEBUG_Disabled;
    ...
    //load the stack with memory pattern.
    while ( pStartOfStack >= pEndOfStack )
    {
      *pStartOfStack = 0xDEADBEEF;
       pStartOfStack--;
    }
